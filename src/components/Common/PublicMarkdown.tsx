...
import PublicMermaid from "./PublicMermaid";
import { usePublicMessage } from "@hooks/usePublicMessage";

export default function PublicMarkdown({ message }: { message: string }) {
  ...
  const { isProcessing } = usePublicMessage();
      ...
      components={{
        code({ node, inline, className, children, ...props }) {
          ...
          if (className === "language-mermaid" && !isProcessing) {
            return <PublicMermaid code={children[0] as string} />;
          }
