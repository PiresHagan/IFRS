...
export const PlaygroundgForm = () => {
  ...
  const { supported: ttsBrowserSupported } = useTextToSpeech();
  ...
  React.useEffect(() => {
    ...
    const ttsEnabledFromStorage = localStorage.getItem("textToSpeechEnabled");
    if (ttsEnabledFromStorage) {
      setTextToSpeechEnabled(JSON.parse(ttsEnabledFromStorage));
    }
  }, []);
            ...
            {ttsBrowserSupported && (
                  <button
                    ...
                    className={`p-1 rounded-md mr-1 ${
                      textToSpeechEnabled ? "text-blue-500" : "text-gray-400"
                    }`}
                  >
