...
import { PlaygroundPublicMessage } from "./PublicMessage";



export const PlaygroundPublicChat = () => {
  const { messages } = usePublicMessage();
                ...
                {messages.length === 0 && (
                  <div>
                    ...
                    <h1 className="text-2xl sm:text-4xl font-semibold text-center text-gray-400 dark:text-gray-600 flex gap-2 items-center justify-center h-screen">
                      ScaffoldGPT Playground
                    </h1>
                  </div>
                )}
                {messages.map((message, index) => (
                  <PlaygroundPublicMessage
                    key={index}
                    {...message}
